...
const buildSessionSubtitle = (
  ...
  buildSessionSubtitleWithTranslation(
    session,
    duration,
    calories,
    i18n.t,
...
describe('workoutSession', () => {
  ...
  describe('buildSessionSubtitle', () => {
    describe('preset sessions', () => {
      ...
      it('includes volume in kg when sets have weight and reps', () => {
        ...
        // 500 + 640 = 1140 kg
        expect(buildSessionSubtitle(session, 60, 300)).toBe(
          `1 exercise · 2 sets · ${fmt(1140)} kg · 300 Cal`
        );
      });
      ...
      it('drops the sets segment entirely for a cardio-only workout', () => {
        ...
        expect(buildSessionSubtitle(session, 55, 500)).toBe(
          '2 exercises · 15.0 km · 500 Cal'
        );
      });
    ...
    describe('individual with multiple sets', () => {
      ...
      it('routes a set-backed cardio activity through the entry totals (never "1 set")', () => {
        const session = makeIndividual({
          distance: 5.2,
          sets: [
            { weight: null, reps: null, duration: 1800, distance: 5.2 },
          ] as any,
        });
        expect(buildSessionSubtitle(session, 30, 300)).toBe(
          '30 min · 5.2 km · 300 Cal'
        );
      });
      ...
      it('routes multi-set cardio through the entry totals too', () => {
        ...
        expect(buildSessionSubtitle(session, 20, 200)).toBe(
          '20 min · 4.0 km · 200 Cal'
        );
      });
    });
      ...
      const payload = buildPresetExercisesPayload(
        [
          ...
          makeDraftExercise({
            exerciseId: 'ex-2',
            sets: [
              { clientId: 's2', weight: '50', reps: '10', distance: '3.11' },
            ],
          }),
        ],
        'kg',
  ...
  describe('buildSessionExercisesPayload', () => {
    ...
    function expectNoTempIds(
      payload: ReturnType<typeof buildSessionExercisesPayload>
    ) {
    ...
    it('passes weight through in kg without conversion', () => {
      ...
      expect(
        buildSessionExercisesPayload(session, {}, {})[0].sets[0].weight
      ).toBe(102.5);
    });
      ...
      const session = makePreset({
        exercises: [
          makeExercise({
            ...
            exercise_id: EX_2,
            sets: [
              makeSet({ id: 201, set_number: 7 }),
              makeSet({ id: 202, set_number: 3 }),
            ],
          }),
    ...
    it('round-trips exercise-level notes and duration_minutes', () => {
      const session = makePreset({
        exercises: [
          makeExercise({ notes: 'superset next time', duration_minutes: 25 }),
        ],
      });
      const payload = buildSessionExercisesPayload(session, {}, {});
    ...
    it('emits explicit nulls for absent exercise notes', () => {
      const session = makePreset({
        exercises: [makeExercise({ notes: null })],
      });
    ...
    it('emits completed_at from the completion map: ISO for mapped ids, null otherwise', () => {
      ...
      expect(payload[0].sets[0].completed_at).toBe(
    ...
    it('carries completed_at on a temp set whose id is omitted', () => {
      ...
      expect(payload[1].sets[0]).not.toHaveProperty('id');
      expect(payload[1].sets[0].completed_at).toBe(
        new Date(completedMs).toISOString()
      );
    });
  ...
  describe('compareSetRecords', () => {
    it('orders by weight at hundredths precision, then reps', () => {
      expect(
        compareSetRecords({ weight: 100, reps: 5 }, { weight: 90, reps: 8 })
      ).toBeGreaterThan(0);
      expect(
        compareSetRecords({ weight: 90, reps: 8 }, { weight: 100, reps: 5 })
      ).toBeLessThan(0);
      // Equal weight → reps break the tie (null reps count as 0).
      expect(
        compareSetRecords({ weight: 100, reps: 6 }, { weight: 100, reps: 5 })
      ).toBeGreaterThan(0);
      expect(
        compareSetRecords({ weight: 100, reps: null }, { weight: 100, reps: 0 })
      ).toBe(0);
    });
  ...
  describe('matchesSetRecord', () => {
    ...
    it('never matches warmups or weightless sets', () => {
      expect(
        matchesSetRecord(
          { weight: 100, reps: 5, set_type: 'Warm-up Set' },
          best
        )
      ).toBe(false);
    ...
    it('is false without a weighted record to tie', () => {
      ...
      expect(
        matchesSetRecord({ weight: 100, reps: 5 }, { weight: null, reps: 5 })
      ).toBe(false);
    });
  });
      ...
      ({
        ...
        exercises: [
          ...
          },
          {
            ...
            sets: [
              { id: 201, set_type: 'drop', weight: 120, reps: 3, rpe: null },
            ],
          },
        ],
      }) as unknown as PresetSession;
    ...
    describe('buildPresetStartExercisesPayload', () => {
      ...
      it('emits exercises that parse under the request schema', () => {
        const preset = makeWorkoutPreset({
          exercises: [
            makePresetExercise({
              sets: [makePresetSet({ reps: null, weight: null })],
            }),
        ...
        for (const exercise of buildPresetStartExercisesPayload(preset)) {
          expect(() =>
            presetSessionExerciseRequestSchema.parse(exercise)
          ).not.toThrow();
        }
      });
    });
  ...
  describe('set metrics', () => {
    ...
    describe('formatRecentSessionSet', () => {
      ...
      it('formats weight × reps, converting for the display unit', () => {
        ...
        expect(formatRecentSessionSet(recentSet(100, 5), 'lbs', i18n.t)).toBe(
          '220.5 × 5'
        );
      });
      ...
      it('handles weight-only and reps-only sets', () => {
        expect(formatRecentSessionSet(recentSet(80, null), 'kg', i18n.t)).toBe(
          '80'
        );
        expect(formatRecentSessionSet(recentSet(null, 12), 'kg', i18n.t)).toBe(
          '12 reps'
        );
      });

      it('localizes reps and fractional presentation in English and Polish', async () => {
        expect(formatRecentSessionSet(recentSet(null, 1), 'kg', i18n.t)).toBe(
          '1 rep'
        );
        expect(formatRecentSessionSet(recentSet(null, 2), 'kg', i18n.t)).toBe(
          '2 reps'
        );
        expect(
          formatSetLoad({ weightKg: 12.3, reps: null }, 'kg', i18n.t)
        ).toBe('12.3 kg');
        expect(
          formatRecentSessionSet(
            { ...recentSet(null, null), duration: 60, distance: 1.25 },
            'kg',
            i18n.t,
            'duration_distance'
          )
        ...
        try {
          ...
          expect(formatRecentSessionSet(recentSet(null, 5), 'kg', i18n.t)).toBe(
            '5 powtórzeń'
          );
          expect(
            formatRecentSessionSet(recentSet(null, 22), 'kg', i18n.t)
          ).toBe('22 powtórzenia');
          expect(
            formatRecentSessionSet(recentSet(null, 25), 'kg', i18n.t)
          ).toBe('25 powtórzeń');
          expect(
            formatSetLoad({ weightKg: 12.3, reps: null }, 'kg', i18n.t)
          ).toBe('12,3 kg');
      ...
      it('shows legacy reps as seconds only for duration modality', () => {
        expect(
          formatRecentSessionSet(recentSet(null, 45), 'kg', i18n.t, 'duration')
        ).toBe('45s');
        expect(
          formatRecentSessionSet(
            recentSet(null, 10),
      ...
      it('renders a dash for all-null sets and seconds without a modality', () => {
        expect(
          formatRecentSessionSet(recentSet(null, null), 'kg', i18n.t)
        ).toBe('–');
        expect(
          formatRecentSessionSet(
            { ...recentSet(null, null), duration: 30 },
            'kg',
            i18n.t
          )
        ).toBe('30s');
    ...
    describe('resolveAssumedSetValues', () => {
      ...
      it('mirrors the rows above onto sets with no history of their own', () => {
        ...
        const added = resolveAssumedSetValues(
          [makeSet(1), makeSet(2)],
          [prev(100, 8)]
        );
      ...
      it('keeps warmup and working mirrors separate', () => {
        ...
        const result = resolveAssumedSetValues(
          [
            makeSet(1, { set_type: 'warmup', weight: 60, reps: 10 }),
            makeSet(2),
            makeSet(3),
          ],
          undefined
        ...
        const warmupAfter = resolveAssumedSetValues(
          [
            makeSet(1, { weight: 100, reps: 5 }),
            makeSet(2, { set_type: 'warmup' }),
          ],
          undefined
        );
        expect(warmupAfter[1]).toEqual({
          weight: null,
          reps: null,
          duration: null,
          distance: null,
      ...
      it('resolves nothing for a brand-new exercise with no sources', () => {
        expect(
          resolveAssumedSetValues([makeSet(1), makeSet(2)], undefined)
        ).toEqual([
      ...
      it('cascades duration through the same previous → planned → preceding chain', () => {
        ...
        expect(legacyPlanned[0]).toEqual({
          weight: 80,
          reps: 5,
          duration: null,
          distance: null,
    ...
    describe('formatSetLoad', () => {
      it('formats weight × reps with the display unit, converting for lbs', () => {
        expect(formatSetLoad({ weightKg: 60, reps: 8 }, 'kg', i18n.t)).toBe(
          '60 kg × 8'
        );
        expect(formatSetLoad({ weightKg: 60, reps: 8 }, 'lbs', i18n.t)).toBe(
          '132.3 lbs × 8'
        );
      });

      it('handles reps-only and weight-only sets', () => {
        expect(formatSetLoad({ weightKg: null, reps: 12 }, 'kg', i18n.t)).toBe(
          '12 reps'
        );
        expect(formatSetLoad({ weightKg: 80, reps: null }, 'kg', i18n.t)).toBe(
          '80 kg'
        );
      });
      ...
      it('returns null when the set has neither weight nor reps', () => {
        expect(
          formatSetLoad({ weightKg: null, reps: null }, 'kg', i18n.t)
        ).toBeNull();
      });
      ...
      it('prefers a duration over weight/reps text', () => {
        ...
        expect(
          formatSetLoad(
            { weightKg: 60, reps: 8, durationSec: 90 },
            'kg',
            i18n.t
          )
        ).toBe('1:30');
    ...
    describe('modality helpers', () => {
      describe('resolveSnapshotModality', () => {
        ...
        it('derives from category when modality is absent or invalid', () => {
          ...
          expect(resolveSnapshotModality({ category: null })).toBe(
            'weight_reps'
          );
        });
      });
      ...
      describe('effectiveSetDurationSec', () => {
        it('prefers the stored duration', () => {
          expect(
            effectiveSetDurationSec({ duration: 30, reps: 45 }, 'duration')
          ).toBe(30);
        });

        it('falls back to legacy reps-as-seconds for duration modality only', () => {
          expect(
            effectiveSetDurationSec({ duration: null, reps: 45 }, 'duration')
          ).toBe(45);
          // Backfilled cardio presets carry seeded reps that must NOT render
          // as seconds.
          expect(
            effectiveSetDurationSec(
              { duration: null, reps: 10 },
              'duration_distance'
            )
          ).toBeNull();
          expect(
            effectiveSetDurationSec({ duration: null, reps: 45 }, 'weight_reps')
          ).toBeNull();
        });

        it('returns null when nothing is available', () => {
          expect(
            effectiveSetDurationSec({ duration: null, reps: null }, 'duration')
          ).toBeNull();
        });
      });
  ...
  describe('exercise reordering', () => {
    ...
    describe('moveSessionExerciseItem', () => {
      ...
      it('moves a solo item down', () => {
        ...
        expect(ids(moveSessionExerciseItem(input, 0, 2))).toEqual([
          'b',
          'c',
          'a',
        ]);
      ...
      it('moves a solo item up', () => {
        const input = [sEntry('a', null), sEntry('b', null), sEntry('c', null)];
        expect(ids(moveSessionExerciseItem(input, 2, 0))).toEqual([
          'c',
          'a',
          'b',
        ]);
      ...
      it('moves a whole run as one indivisible block', () => {
        ...
        expect(ids(moveSessionExerciseItem(input, 1, 0))).toEqual([
          'a',
          'b',
          'x',
          'y',
    ...
    describe('moveDraftExerciseItem', () => {
      ...
      it('derives items identically to the session mover (mirrored order/groups)', () => {
        ...
        const session = draft.map((e) =>
          sEntry(e.clientId, e.supersetGroup ?? null)
        );
  ...
  describe('buildPresetUpdateExercises', () => {
    ...
    it('emits exercises that parse under the preset request schema', () => {
      ...
      for (const exercise of payload) {
        expect(() =>
          workoutPresetExerciseRequestSchema.parse(exercise)
        ).not.toThrow();
      }
    });
      ...
      const session = makePreset({
        exercises: [
          makeSessionExercise({
            sets: [
              makeSessionSet(),
              makeSessionSet({ id: 102, set_number: 2, weight: 90 }),
            ],
          }),
        ],
      });
    ...
    it('returns null when the only open question is a skipped set still matching its plan', () => {
      const session = makePreset({
        exercises: [
          makeSessionExercise({
            sets: [makeSessionSet({ weight: null, reps: null })],
          }),
        ],
      });
      expect(
    ...
    it('treats an explicitly cleared note on a completed set as a deviation, not a resurrection', () => {
      ...
      const preset = makeTargetPreset({
        exercises: [
          makeTargetPresetExercise({
            sets: [makeTargetPresetSet({ notes: 'slow tempo' })],
          }),
        ],
      });
      const payload = buildPresetUpdateExercises(
    ...
    it('nulls duration on non-duration modalities so leaked values are not deviations', () => {
      ...
      const session = makePreset({
        exercises: [
          makeSessionExercise({ sets: [makeSessionSet({ duration: 90 })] }),
        ],
      });
      const preset = makeTargetPreset({
        exercises: [
          makeTargetPresetExercise({
            sets: [makeTargetPresetSet({ duration: 45 })],
          }),
        ],
      });
      expect(
    ...
    it('keeps duration on duration-modality exercises and detects a changed hold', () => {
      ...
      const preset = makeTargetPreset({
        exercises: [
          makeTargetPresetExercise({
            sets: [
              makeTargetPresetSet({ reps: null, weight: null, duration: 60 }),
            ],
          }),
        ],
      });
    ...
    it('nulls distance on non-cardio modalities so junk values are not deviations', () => {
      const session = makePreset({
        exercises: [
          makeSessionExercise({ sets: [makeSessionSet({ distance: 3 })] }),
        ],
      });
      const preset = makeTargetPreset({
        exercises: [
          makeTargetPresetExercise({
            sets: [makeTargetPresetSet({ distance: 7 })],
          }),
        ],
      });
      expect(
    ...
    it('canonicalizes an untouched fabricated set back to a zero-set preset exercise', () => {
      ...
      const preset = makeTargetPreset({
        exercises: [makeTargetPresetExercise({ sets: [] })],
      });
      expect(
        buildPresetUpdateExercises(session, preset, {
          completedSetIds: {},
    ...
    it('writes the fabricated set into a zero-set preset exercise once it was actually used', () => {
      const session = makePreset({
        exercises: [
          makeSessionExercise({
            sets: [makeSessionSet({ weight: 80, reps: 10 })],
          }),
        ],
      });
      const preset = makeTargetPreset({
    ...
    it('treats a null set_type as normal on both sides', () => {
      const session = makePreset({
        exercises: [
          makeSessionExercise({ sets: [makeSessionSet({ set_type: null })] }),
        ],
      });
      const preset = makeTargetPreset({
        exercises: [
          makeTargetPresetExercise({
            sets: [makeTargetPresetSet({ set_type: 'normal' })],
          }),
        ],
      });
      expect(
    ...
    it('passes kg through with float-noise normalization instead of a unit round-trip', () => {
      const session = makePreset({
        exercises: [
          makeSessionExercise({
            sets: [makeSessionSet({ weight: 102.50000000001 })],
          }),
        ],
      });
      const preset = makeTargetPreset({
        exercises: [
          makeTargetPresetExercise({
            sets: [makeTargetPresetSet({ weight: 102.5 })],
          }),
        ],
      });
      expect(
